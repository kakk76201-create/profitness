...
#
# ПОРЯДОК КЛЮЧЕЙ ВАЖЕН: monthly, quarterly, yearly, lifetime — от короткого к
# длинному. В этом порядке tariff_catalog() отдаёт тарифы фронту, и витрина
# строит «лесенку»: чем дольше срок, тем дешевле обходится месяц. Новый тариф
# вставляется на своё место по сроку, а не в конец.
TARIFFS: dict = {
    "monthly": {
        "days": int(os.getenv("SUBSCRIPTION_MONTHLY_DAYS", "30")),
    },
    # «3 месяца» — промежуточная ступень между месяцем и годом для тех, кому
    # страшно сразу платить за год, но уже понятно, что месяца мало.
    "quarterly": {
        "days": int(os.getenv("SUBSCRIPTION_QUARTERLY_DAYS", "90")),
    },
    "yearly": {
...
# Название продукта в описании платежа. Его видят плательщик (в форме оплаты,
# в выписке и в чеке) и модератор платёжного сервиса, который сверяет описание
# с витриной — поэтому здесь имя бота/приложения, а не внутреннее прозвище.
PAYMENT_PRODUCT_NAME = "Fitness Up"

# Человекочитаемые названия тарифов для описания платежа. Код тарифа
# ("yearly") плательщику ничего не говорит и выглядит как ошибка, поэтому
# в описание идёт только русское название. Словарь лежит рядом с TARIFFS,
# чтобы новый тариф не забыли назвать: оба провайдера берут название отсюда.
TARIFF_TITLES_RU: dict = {
...
def payment_description(tariff: str) -> str:
    """Описание платежа для плательщика: «Fitness Up — подписка на 3 месяца».

    Для незнакомого тарифа — нейтральное «подписка»: код тарифа наружу не
    показываем (до этой функции неизвестный тариф всё равно не доходит —
    маршруты оплаты отсекают его раньше).
    """
    title = TARIFF_TITLES_RU.get(tariff, "подписка")
    return f"{PAYMENT_PRODUCT_NAME} — {title}"
...
#
# Лесенка по умолчанию честная — чем дольше срок, тем дешевле месяц:
#   месяц    699 ₽;
#   3 месяца 1790 ₽ (≈597 ₽/мес, экономия 15% против 3 × 699);
#   год      5590 ₽ (≈466 ₽/мес, экономия 33% против 12 × 699).
# Меняя одну цену, проверь, что ступени не «перевернулись»: витрина считает
# экономию относительно месячной цены и при нулевой выгоде строку не покажет.
RUB_PRICES: dict = {
    "monthly": float(os.getenv("PRICE_MONTHLY_RUB", "699") or 0),
    "quarterly": float(os.getenv("PRICE_QUARTERLY_RUB", "1790") or 0),
...
def tariff_catalog() -> dict:
    ...
    Порядок ключей — как в TARIFFS (monthly, quarterly, yearly, lifetime):
    JSON сохраняет его, и по нему идут карточки на витрине.
    """
    catalog = {}
